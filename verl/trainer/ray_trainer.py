...
def compute_advantage(data: DataProto, adv_estimator: AdvantageEstimator, gamma: float = 1.0, lam: float = 1.0, balance_beta: float = 1.0):
    ...
    adv_inputs = {
        ...
        "gamma": gamma,
        "lam": lam,
        "partition_id": data.non_tensor_batch["partition_id"] if "partition_id" in data.non_tensor_batch else None,
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.emb_client = OpenAI(
            base_url="http://localhost:8000/v1", # Replace with your server address
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        num_to_generate = 0 # current generated number of examples
        preserve_ratio = 0 # ratio of preservation of dynamic sampling
        ...
        while True:
            ...
            try:
                ...
                if num_try_make_batch > 1:
                    scaled_rollout_batch_size = int(num_to_generate / (preserve_ratio - 0.1)) # scale down to sufficient number of samples
                    scaled_rollout_batch_size += self.config.trainer.n_gpus_per_node - (scaled_rollout_batch_size % self.config.trainer.n_gpus_per_node) # make it divisible by n_gpus_per_node
                    for key in batch_dict:
                        batch_dict[key] = batch_dict[key][:scaled_rollout_batch_size]
            ...
            if self.config.algorithm.online_filtering:
                ...
                # partition groups
                #new_batch = self._partition_groups(new_batch)
            ...
            num_to_generate = rollout_batch_size - current_batch_size
            if num_try_make_batch == 1:
                preserve_ratio = current_batch_size / rollout_batch_size
    ...
    def _partition_groups(self, batch: DataProto) -> DataProto:
        uids = batch.non_tensor_batch["uid"]
        reasoning = batch.non_tensor_batch["reasoning"]
        ...
        # Placeholder for partition ids, same length as batch
        partition_ids = np.zeros(len(uids), dtype=int)
        all_embeddings = [None] * len(uids)

        # We'll assign partition_id starting from 1 within each uid group
        for uid, indices in uid_to_indices.items():
            # extract the reasoning for this group
            group_reasoning = [reasoning[i] for i in indices]

            # embed reasoning
            response = self.emb_client.embeddings.create(
                model="Qwen/Qwen3-Embedding-0.6B", # Or the model you are serving
                input=group_reasoning
            )
            embeddings = [embedding.embedding for embedding in response.data]

            # Save embeddings back into all_embeddings by batch index
            for local_idx, batch_idx in enumerate(indices):
                all_embeddings[batch_idx] = embeddings[local_idx]

            clustering = KMeans(n_clusters=self.config.algorithm.partition_k, size_min=self.config.algorithm.partition_min, random_state=0).fit(embeddings)
            labels = clustering.labels_

            # Assign partition ids, starting from 1
            unique_labels = {lab: idx + 1 for idx, lab in enumerate(np.unique(labels))}
            for i, idx in enumerate(indices):
                partition_ids[idx] = unique_labels[labels[i]]

        # Add partition_ids and embeddings to the non_tensor_batch
        batch.non_tensor_batch["partition_id"] = partition_ids
        batch.non_tensor_batch["embedding"] = np.array(all_embeddings, dtype=object)
    ...
    def fit(self):
        ...
        while self.global_step < self.training_steps:
            ...
            with timer("step", timing_raw):
                ...
                #     batch = self._partition_groups(batch)
                ...
                if "token_level_scores" not in batch.batch:
                    with timer("reward", timing_raw):
                        reward_ref = self.reward_fn.compute_reward.remote(
                            ...
                            lambda_init=self.config.algorithm.lambda_init, 
                            decay_rate=self.config.algorithm.decay_rate
                        )
                ...
                with timer("adv", timing_raw):
                    ...
                    batch = compute_advantage(
                        ...
                        balance_beta=self.config.algorithm.balance_beta,
                    )
