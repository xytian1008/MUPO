...
# NOTE(sgm): this implementation only consider outcome supervision, where the reward is a scalar.
@register_adv_estimator(AdvantageEstimator.MUPO)
def compute_mupo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6, partition_id: np.ndarray = None, balance_beta: float = 1.0, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Compute advantage for MUPO, operating only on Outcome reward (with only one scalar reward for each response).
    ...
    id2score = defaultdict(list)
    id2mean, id2std = {}, {}
    id2count_per_uid = defaultdict(list)
    group_counts = {}  # (uid, partition_id): count in this group

    bsz = scores.shape[0]
    # Calculate advantage within the same uid AND same partition_id
    for i in range(bsz):
        key = (index[i], partition_id[i])
        id2score[key].append(scores[i])

    # For each group, record the count and for each uid, record all group counts
    for key in id2score:
        uid = key[0]
        count = len(id2score[key])
        group_counts[key] = count
        id2count_per_uid[uid].append(count)

    # Compute mean and std for each group
    for key in id2score:
        assert len(id2score[key]) > 1, "MUPO needs rollout.n > 1 for the same (uid, partition_id)."
        values_tensor = torch.stack(id2score[key])
        id2mean[key] = torch.mean(values_tensor)
        id2std[key] = torch.std(values_tensor)

    # Compute group weights according to the formula: (avg_num / current_num) ^ balance_beta
    group_weights = {}
    for key in id2score:
        uid = key[0]
        counts = id2count_per_uid[uid]
        avg_count = float(sum(counts)) / float(len(counts))
        count = group_counts[key]
        group_weights[key] = (avg_count / count) ** balance_beta

    # Apply normalization and weight
    for i in range(bsz):
        key = (index[i], partition_id[i])
        normed = (scores[i] - id2mean[key]) / (id2std[key] + eps)
        w = group_weights[key]
        scores[i] = normed * w
