...
def _extract_think(resp):
    # Try to find <think>...</think>
    match = re.search(r'(<think>.*?</think>)', resp, flags=re.DOTALL)
    if match:
        return match.group(1)
    else:
        # Fallback: everything before '\boxed'
        boxed_idx = resp.find(r'\boxed')
        if boxed_idx != -1:
            return resp[:boxed_idx]
        else:
            return resp  # fallback to full response if no match
...
class vLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        with self.update_sampling_params(**prompts.meta_info):
            ...
            reasoning = [_extract_think(resp) for resp in responses]
        ...
        if batch_multi_modal_data is not None:
            non_tensor_batch = {"multi_modal_data": batch_multi_modal_data,
                                "reasoning": np.array(reasoning, dtype=object)}
        else:
            non_tensor_batch = {"reasoning": np.array(reasoning, dtype=object)}
